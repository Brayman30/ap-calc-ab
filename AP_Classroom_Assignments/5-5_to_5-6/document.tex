...
\lhead{AP Calculus AB -- APC 5.5 -- 5.6 Assignment\\}
\rhead{Brayden Price\\}
...
\begin{document}
	...
	\LARGE AP Classroom: 5.5 -- 5.6 \\[0.5em]
	\large \today\\[0.5em]
	\large Brayden Price \\
	...
	\begin{questions}
		...
		\begin{solution}
			\begin{parts}
				...
				&= \boxed{\frac{2\pi\cos(\pi t)-\pi t \cos (\pi t) + \sin(\pi t)}{(2-t)^2}}
				\end{align}
				\pagebreak
				\part
		...
		\begin{solution}
			...
			\begin{align}
				2e^{2x} = 0 		\ \   &| \ \  (1+2x) = 0 \\
				\ln (e^2x) = 0 		\ \   &| \ \ 2x = -1 \\
				\text{no solution}	\ \   &| \ \ x = -\frac{1}{2}
			\end{align}
			Sign chart of $f^\prime(x)$:
			$$\sgchart{-\frac{1}{2}}{f^\prime(x):-+}$$
			...
			\end{align}
			Conclusion: \\
			Since $x=-\frac{1}{2}$ is the only relative minima and is less than the $\lim_{x\to \pm \infty} (f(x))$, it is the absolute minimum of $f(x)$.
			$$f(-1) = 2(-1)e^{2(1)}= -2e^{2}$$
		...
		\begin{solution}
			\begin{parts}
				\part $$A^\prime(t) \approx \frac{A(30)-A(15)}{30-15} = \frac{16-25}{30-15} = - \frac{9}{15} = \boxed{- \frac{3}{5}}$$
